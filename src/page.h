...
void add_tx_metadata(mstch::map &context, const cryptonote::transaction &tx, bool detailed = false) {
    ...
    else if (tx.version >= txversion::v4_tx_types && tx.type == txtype::vaizon_name_system)
    {
      tx_extra_vaizon_name_system lns_extra = {};
      if (get_vaizon_name_system_from_tx_extra(tx.extra, lns_extra))
      {
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_vaizon_network_info(j_info);
...
json_version()
{
    ...
    j_data = json {
            ...
            {"vaizon_version_full"   , string {VAIZON_VERSION_FULL}},
...
bool
get_vaizon_network_info(json& j_info)
{
