# Vaizon Onion Blockchain Explorer
 ...
 **Blocks and transactions**
 ...
 - shows public components of Vaizon addresses,
 - decoding which outputs and mixins belong to the given Vaizon address and viewkey,
 - can prove that you send Vaizon to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - shows number of amount output indices,
 - support Vaizon testnet, stagenet network,
...
Current development branch:
 ...
 - https://github.com/Vaizon-Project/onion-vaizon-blockchain-explorer/tree/test



## Compilation on Ubuntu 16.04/18.04

##### Compile latest Vaizon master version

Download and compile recent Vaizon into your home folder:

```bash
# first install Vaizon dependecines
sudo apt update
...
Once the Vaizon is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/vaizon
cd ~

# download the source code
git clone https://github.com/Vaizon-Project/vaizon-onion-blockchain-explorer.git

# enter the downloaded sourced code folder
cd vaizon-onion-blockchain-explorer
...
# alternatively can use: cmake -DVAIZON_DIR=/path/to/vaizon ..
# if vaizon is not in ~/vaizon
#
...
By default it will look for blockchain in its default location i.e., `~/.vaizon/lmdb`.
...
```
lokblocks, Onion Vaizon Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Vaizon total emission monitoring
                                        thread
  ...
  --ssl-key-file arg                    path to key file for ssl (https)
                                        functionality
  -d [ --daemon-url ] arg (=http:://127.0.0.1:13301)
                                        Vaizon daemon url
```
...
alias lokblocksmainnet='~/vaizon-onion-blockchain-explorer/build/lokblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias lokblockstestnet='~/vaizon-onion-blockchain-explorer/build/lokblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Vaizon emission 

Obtaining current Vaizon emission amount is not straight forward. Thus, by default it is 
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.vaizon/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.vaizon/testnet/lmdb/emission_amount.txt` or `~/.vaizon/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.vaizon/lmdb/emission_amount.txt`
...
```
Vaizon emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Vaizon daemon's  `print_coinbase_tx_sum` command. 
...
Having the `crt` and `key` files, run `vznblocks` in the following way:

```bash
./vznblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_vaizon",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "vaizon_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Vaizon.
...
Return raw json tx data, as represented in Vaizon.
